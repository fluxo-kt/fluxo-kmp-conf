...
- `androidLibrary { }` inside `fkcSetupMultiplatform` now fails fast with a migration message under AGP 9+, instead of letting AGP itself reject the `com.android.library` + `kotlin("multiplatform")` co-application with a cryptic error. The message names the supported replacement (apply `com.android.kotlin.multiplatform.library` and use `kotlin { android { } }` directly). AGP-8 builds are unaffected.
