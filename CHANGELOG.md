...
### Known limitations
- KMP modules with an Android target: AGP 9.0+ rejects the legacy `com.android.library` + `kotlin("multiplatform")` combination in favour of the new `com.android.kotlin.multiplatform.library` plugin (which auto-creates the `android` KMP target). The plugin's wrap surface still applies the legacy `com.android.library` for KMP+Android targets — wider DSL refactor scheduled for a 0.14.x patch. **Workaround for affected consumers**: pin AGP to the latest 8.x line (`android-gradle-plugin = "8.13.0"`) in your version catalog until the migration ships, or apply `com.android.kotlin.multiplatform.library` directly in your build script in addition to enabling the Android target via `fkcSetupMultiplatform`.
...
- bump **Compose Multiplatform 1.7.1 → [1.10.3](https://github.com/JetBrains/compose-multiplatform/releases/tag/v1.10.3)** — common `@Preview` annotation, Navigation 3, stable Compose Hot Reload. Compose Compiler 1.10's deprecated `enableStrongSkippingMode` / `enableIntrinsicRemember` / `enableNonSkippingGroupOptimization` knobs still work but emit Compose-side deprecation warnings (the new `featureFlags` API will land in a follow-up).
