...
- bump `com.vanniktech.maven.publish` [_0.30.0 → 0.35.0_](https://github.com/vanniktech/gradle-maven-publish-plugin/releases/tag/0.35.0). The upstream `publishToMavenCentral(host, automaticRelease)` overload and the `SonatypeHost` enum were removed in 0.34.0 (Sonatype OSSRH retired 2025-06-30); 0.35.0 is the latest Vanniktech that still ships Kotlin 1.9.x metadata (`mv=[1,8,0]`) and is readable by our current build Kotlin. 0.36.0 bumps to Kotlin 2.2 metadata and is held back until the build Kotlin upgrade lands. See the `sonatypeHost` removal in **Removed**.
