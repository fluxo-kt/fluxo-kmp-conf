...
### Removed
- drop the `kotlinx.atomicfu.enable{Js,Jvm,Native}IrTransformation` flags from the root `gradle.properties`. The kotlinx-atomicfu plugin is not applied anywhere in this repository, so the flags were silent no-ops; deleting them avoids implying coverage that does not exist.
- delete the unused `ABOUT_LIBRARIES` integration scaffolding (constant + `buildConfigField`). Two years with zero source consumers; the original `// FIXME: Add support for plugin` comment was a stale intent. Re-introduce when an actual integration ships.
- drop dead constant arrays from `fluxo.conf.data.VersionCatalogConstants`. The remaining alias constants are documented as the live API surface.
- delete the dead `actions/cache` step in `build.yml` (gated `if: false` with an outdated `-kmp-2.0.0` cache key).

### Added
- enforce build-script mirror invariance with a new `verifyBuildScriptMirror` task wired to `:plugin:check`. The task byte-diffs the `// MIRROR-START` / `// MIRROR-END` regions of `fluxo-kmp-conf/build.gradle.kts` and `self/build.gradle.kts`. Replaces the previous "discipline only" mirror policy with a CI-enforced invariant; cached via `outputs.upToDateWhen { true }` so cached `:plugin:check` runs are still UP-TO-DATE.
- one-shot warning when the Kotlin plugin outruns the hand-maintained Kotlin → max-JVM compatibility table. Surfaces silent JVM-target capping at configuration time so the maintainer notices an upstream Kotlin minor that needs a table extension.
- one-shot warning when the requested Kotlin language version outruns Detekt's supported maximum. Replaces the prior silent clamp and points the maintainer at the exact constant to update.
- expose Gradle Doctor as a configurable escape hatch on the existing `enableGradleDoctor` toggle (KDoc-only documentation of the lambda; no public surface change).
- maintainer guide at `AGENTS.md` (with `CLAUDE.md` compatibility symlink). Targets contributors of the plugin, not consumers; `README.md` remains the consumer entry point.
- RED-test seeds for the project's most fragile reflective and table-driven surfaces: Kotlin version table + plugin-version parser (with at-AND-just-below boundary cases that falsify `>=`-to-`>` mutations), Detekt clamp comparator (with two-digit-minor cases), `MergeDetektBaselinesTask` reflective `BaselineProvider` / `BaselineFormat` fallback, and the shrinker reflective entrypoints. A future upstream rename or removal turns one test RED at the exact line that needs updating.
- KDoc for `projectName` documenting the Maven artifact-ID restriction.

### Changed
- migrate publication archive setup from the removed-in-Gradle-9.0 `dirMode` / `fileMode` integer setters to `dirPermissions { unix(...) }` / `filePermissions { unix(...) }` (added in Gradle 8.3). Retains a `NoSuchMethodError` fallback for true 8.0–8.2 holdouts. Eliminates one Gradle 9 deprecation surface ahead of the wrapper bump.
- align the lone outlier `configureExtension<DependencyAnalysisExtension>()` caller with the codebase convention (named-form, `name = "dependencyAnalysis"`); 13 of 14 callers already used it. Drops one Gradle internal-deprecation warning.
- decouple `JarState` and `LazyForwardingEquality` from Spotless internals (`com.diffplug.spotless.{FileSignature,NoLambda}`) by inlining ~25 LOC equivalents. Removes a brittle private-API dependency that would have broken on the Spotless 7.x bump.
- extract pure-Kotlin version-table and Detekt clamp helpers (`KotlinVersionTable.kt`, `DetektKotlinClamp.kt`) from their KGP-coupled host files. Required to make the new RED tests runnable without dragging the KGP `compileOnly` classpath into the test runtime.
- rename `DependencyUpdatesPlugin.kt.kt` → `.kt` (typo).

### Fixed
- the table-staleness warning was firing on every consumer build with Kotlin 2.0.x patch ≥ 1. The predicate compared `currentKotlin > KOTLIN_2_0` against `KotlinVersion`'s patch-inclusive `compareTo`, so any patch above 0 falsely "outran the table" even though the `>= KOTLIN_2_0` JVM-target bracket already covered all 2.0.x patches. Renamed the constant to `FIRST_UNTABULATED_KOTLIN = KOTLIN_2_1` and switched to `>=` so the warning fires exactly when a new minor needs a table entry.
- the Detekt language-version clamp used `requested.toFloat() <= max.toFloat()`, which works only while every Kotlin minor is single-digit. `"1.10".toFloat() == 1.1f` ranked falsely *below* 1.9; `"2.10".toFloat() == 2.1f` ranked falsely *equal* to 2.1 with no clamp. Replaced with a `kotlin.KotlinVersion`-based comparator (no KGP coupling). Latent today (no Kotlin 1.10/2.10 yet) but a correctness landmine.
- regenerate `api/plugin.api` and `pg/keep-api-autogenerated.pro` against current source; both had drifted ~17 months behind (still listing the long-removed `SonatypeHost` component on `FluxoPublicationConfig`).
- correct the misleading `@Deprecated` note on the BCV `API_DIR` copy. BCV 0.16.3 exposes no public top-level `API_DIR`; the proper rewire target is the per-project `ApiValidationExtension.apiDumpDirectory` and is scheduled with the BCV bump.

### Updated
- expand the CI JDK matrix to `['21', '23']` so the pre-existing `matrix.java != '23'` CodeQL gate (which had been silently dead on a single-JDK matrix) now actually runs on the JDK-21 leg.
- pin supply-chain references that were previously mutable: `docker://antonyurchenko/git-release@sha256:b3a7d6…` (v6.0.0 multi-arch manifest digest) and `gh extension install actions/gh-actions-cache --pin <commit>` (v1.0.4). Bump procedures inlined alongside each pin.
- group and cool down Dependabot updates so coupled bumps land atomically: `kotlin` / `agp` / `compose` / `detekt` / `vanniktech` / `dokka` / `proguard-r8` / `build-tooling` groups for Gradle deps, single `gh-actions` group for workflows, 7-day stability cooldown (30 days for major bumps). Stops the catalog-drifts-three-Kotlin-minors-behind failure mode at the boundary.
- pin remaining unpinned action versions across the workflows (`action-up`).
