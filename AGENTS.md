...
- `checks/{kmp,gradle-plugin,compose-desktop}/` — three **independent Gradle builds** (own `gradlew`, own `settings.gradle.kts`) integration-testing the plugin against real shapes. CI cd's into each.
...
- **`self/build.gradle.kts` and `fluxo-kmp-conf/build.gradle.kts` carry near-identical `dependencies {}` and `buildConfig {}` blocks** by design (FIXME noted). Every new wrapped plugin / dep **must** be mirrored to both. **No automated guard exists** — discipline only.
