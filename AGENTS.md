...
- **`setupAndroidCommon` (legacy 200-LOC `TestedExtension.()` body) is broken under AGP-9 + non-KMP `com.android.library`** as of 0.14.0. The legacy AGP-8 path still works; the AGP-9 KMP+Android line bypasses it entirely via `setupKmpAndroidExtension`. Migration target documented in the SOP: extract a shared `applyAndroidSlotFills(CommonExtension, conf)` helper (namespace/compileSdk/minSdk/buildToolsVersion/Room) reusable across all three Android setup paths; specialise buildTypes/buildFeatures on concrete `LibraryExtension`/`ApplicationExtension` modern subtypes. Pair the migration with a `checks/android/` non-KMP integration module to surface this class of bug structurally. AGP-8 KMP+Android coverage gap is the symmetric problem on the other AGP line — needs a sibling check or a build-property toggle in `checks/kmp/`.
